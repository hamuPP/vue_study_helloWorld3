 ...
 */
Format.prototype.labelIndex_ = 0;
Format.prototype.labelIndex = 0;

/**
 * Returns information about the current selection.
 */
Format.prototype.currentIndex_ = 0;
...
Format.prototype.refresh = function () {
  ...
  if (graph.isSelectionEmpty()) {
    ...
    var label_ = label.cloneNode(false);

    var currentLabel_ = null;
    var currentPanel_ = null;

    var addClickHandler_ = mxUtils.bind(this, function (elt, panel, index) {
      var clickHandler_ = mxUtils.bind(this, function (evt) {
        if (currentLabel_ != elt) {
          if (containsLabel) {
            this.labelIndex_ = index;
          } else {
            this.currentIndex_ = index;
          }

          if (currentLabel_ != null) {
            mxUtils.removeElementClass(currentLabel_, 'active');
            currentLabel_.style.borderBottomWidth = '1px';
          }

          currentLabel_ = elt;
          currentLabel_.style.borderBottomWidth = '0px';

          currentLabel_.className += ' active';

          if (currentPanel_ != panel) {
            if (currentPanel_ != null) {
              currentPanel_.style.display = 'none';
            }

            currentPanel_ = panel;
            currentPanel_.style.display = '';
          }
        }
      });

      mxEvent.addListener(elt, 'click', clickHandler_);
      // Prevents text selection
      ...
      if (index == ((containsLabel) ? this.labelIndex_ : this.currentIndex_)) {
    ...
    if (!containsLabel) {
      label_.className = 'geFormatSection geFormatSection-label';
      mxUtils.write(label_, mxResources.get('diagram'));
      label_.style.borderLeftWidth = '0px';

      div.appendChild(label_);
      var diagramFormatPanel = div.cloneNode(false);
      diagramFormatPanel.style.display = 'none';
      this.panels.push(new DiagramFormatPanel(this, ui, diagramFormatPanel));
      this.container.appendChild(diagramFormatPanel);

      addClickHandler_(label_, diagramFormatPanel, idx_++);
    }

    // 加入我的面板(非激活状态): 工作流编辑
    var label7 = label.cloneNode(false);
    mxUtils.write(label7, '工作流');
    div.appendChild(label7);

    var myWorkFlowPanel = div.cloneNode(false);
    myWorkFlowPanel.style.display = 'none';
    this.panels.push(new MyWorkFlowPanel(this, ui, myWorkFlowPanel));
    this.container.appendChild(myWorkFlowPanel);

    // 加入我的面板(非激活状态): 工作流的参数配置
    var label8 = label.cloneNode(false);
    mxUtils.write(label8, '工作流参数');
    div.appendChild(label8);

    var myWorkFlowParamsPanel = div.cloneNode(false);
    myWorkFlowParamsPanel.style.display = 'none';
    this.panels.push(new MyWorkFlowParamsPanel(this, ui, myWorkFlowParamsPanel));
    this.container.appendChild(myWorkFlowParamsPanel);

    addClickHandler_(label7, myWorkFlowPanel, idx_++);
    addClickHandler_(label8, myWorkFlowParamsPanel, idx_++);
  ...
  // 点击了具体某个节点时
    ...
    var label9 = label2.cloneNode(false);
...
// 我的自定义节点属性模板
MyNodeAttributePanel = function (format, editorUi, container) {
  BaseFormatPanel.call(this, format, editorUi, container);
  this.init();
};
mxUtils.extend(MyNodeAttributePanel, BaseFormatPanel);
...
MyNodeAttributePanel.prototype.init = function () {
  ...
  var dlg = new EditDataDialogInline(ui, mxCell);
  this.container.appendChild(dlg.container);
};
MyNodeAttributePanel.prototype.addMyInput1 = function (container) {
  var ui = this.editorUi;
  var editor = ui.editor;
  var graph = editor.graph;
  var ss = this.format.getSelectionState();
...
MyNodeAttributePanel.prototype.addMyInput2 = function (container) {
  var ui = this.editorUi;
  var editor = ui.editor;
  var graph = editor.graph;
  var ss = this.format.getSelectionState();
...
// 我的自定义工作流编辑参数
MyWorkFlowPanel = function (format, editorUi, container) {
  BaseFormatPanel.call(this, format, editorUi, container);
  this.init();
};
mxUtils.extend(MyWorkFlowPanel, BaseFormatPanel);

MyWorkFlowPanel.prototype.init = function () {
  // 当前操作的节点的数据们：
  var ui = this.editorUi;
  var ss = this.format.getSelectionState();
  var mxCell = ui.editor.graph.getModel().getRoot();
  var dlg = new EditDataDialogInline_workflow(ui, mxCell);
  this.container.appendChild(dlg.container);
};

// 我的自定义工作流的参数辑参数
MyWorkFlowParamsPanel = function (format, editorUi, container) {
  BaseFormatPanel.call(this, format, editorUi, container);
  this.init();
};
mxUtils.extend(MyWorkFlowParamsPanel, BaseFormatPanel);

MyWorkFlowParamsPanel.prototype.init = function () {
  // 当前操作的节点的数据们：
  var ui = this.editorUi;
  var ss = this.format.getSelectionState();
  var mxCell = ui.editor.graph.getModel().getRoot();
  var dlg = new EditDataDialogInline_wfParams(ui, mxCell);
  this.container.appendChild(dlg.container);
};
/**
 * Adds the label menu items to the given menu and parent.
