...
(function()
{
	...
	EditorUi.prototype.doSaveLocalFile = function(data, filename, mimeType, base64Encoded, format)
	{
	  debugger;
	  // 默认的保存格式为xml
		if (mimeType == 'text/xml' && !/(\.xml)$/i.test(filename))
		{
...
//		}
	...
	EditorUi.prototype.setGraphEnabled = function(enabled)
	{
		...
		this.sidebarFooterContainer && (this.sidebarFooterContainer.style.display = (enabled) ? '' : 'none');
	...
	EditorUi.prototype.updateUi = function()
	{
		...
		var restricted = (urlParams['embed'] != '1' || !this.editor.graph.isEnabled())
      &&
